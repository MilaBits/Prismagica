...
namespace Systems
{
	public class RadialColorMenu : MonoBehaviour
	{
		...
		[SerializeField]
		private Disc background;

		[SerializeField]
		private Disc picked;

		[SerializeField]
		private Disc pickedBackground;

		[SerializeField]
		private Disc indicator;
		...
		[SerializeField]
		private float _darkenAmount = .5f;
		...
		[SerializeField]
		private float _backgroundThickness;

		[SerializeField]
		private float _colorRadius;

		[SerializeField]
		private float _colorThickness;

		[Space]
		[SerializeField]
		private float _pickedBackgroundRadius;

		[SerializeField]
		private float _pickedBackgroundThickness;

		[SerializeField]
		private float _pickedRadius;

		[SerializeField]
		private float _pickedThickness;
		...
		{
			Vector2 relativeMouseDirection = UnityEngine.Camera.main.ScreenToWorldPoint(Input.mousePosition) - transform.position;
		...
		private void DrawWheel()
		{
			...
			foreach (ColorMenuItem colorMenuItem in colorItems)
			{
				...
				colorMenuItem.visualPiece.AngRadiansEnd =
					((currentDegrees + (singleWeightDegrees * colorMenuItem.weight)) * Mathf.Deg2Rad) - space * Mathf.Deg2Rad;
